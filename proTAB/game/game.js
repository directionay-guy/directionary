...
var GAME_URL = "ITCH_URL_HERE"; // Update with itch.io PRO page URL when live
...
function showDailyCompleteModal() {
    ...
    lastCompletedResults = roundResults.slice();
    lastCompletedScore = displayScore;
    lastCompletedMode = gameMode;
    ...
    if (roundResults.length > 0) {
        ...
        summary += '<a href="https://directionary.net" target="_blank" style="display: block; padding: 6px 10px; font-size: 0.72em; font-weight: 600; background: white; color: #888; border: 2px solid #ddd; border-radius: 12px; cursor: pointer; text-decoration: none; text-align: center;">Also play Directionary — free daily game</a>';
        summary += '</div>';
    } else {
...
function generateShareText() {
    ...
    var modeLabel = lastCompletedMode === 'proplus' ? ' PRO+' : ' PRO';
    var text = "Directionary" + modeLabel + "\n\n";
...
function showResultsPanel() {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (result.score === 0) {
            ...
            html += '<div style="text-align: right; color: #666; font-size: 0.8em;">' + result.score + ' pts</div>';
        }
    }
    html += '</div>';
    ...
    html += '<a href="https://directionary.net" target="_blank" style="display: block; padding: 8px 12px; font-size: 0.75em; font-weight: 600; background: white; color: #888; border: 2px solid #ddd; border-radius: 12px; cursor: pointer; text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Also play Directionary — free daily game</a>';
    html += '</div>';
    html += '</div>';
...
function updateStatsDisplay() {
    // Check if periods have rolled over even if no game was played today
    var dayKey = getDateKey(), weekKey = getWeekKey(), monthKey = getMonthKey(), yearKey = getYearKey();
    ['pro', 'proplus'].forEach(function(mode) {
        var s = playerStats[mode];
        if (s.currentDay !== dayKey) { s.dailyTotal = 0; s.currentDay = dayKey; }
        if (s.currentWeek !== weekKey) { s.weeklyTotal = 0; s.currentWeek = weekKey; }
        if (s.currentMonth !== monthKey) { s.monthlyTotal = 0; s.currentMonth = monthKey; }
    });
    saveStats();
...
function switchToInfoTab() {
    ...
    document.getElementById('infoTabBtn').classList.add('active');
    document.getElementById('gameTabBtn').classList.remove('active');
    hideKeyboard();
}
...
function switchToGameTab() {
    ...
    document.getElementById('gameTabBtn').classList.add('active');
    document.getElementById('infoTabBtn').classList.remove('active');
    if (isTouchDevice) showKeyboard();
    if (!isTouchDevice) {
        setTimeout(function() {
            var inp = document.getElementById('guessInput');
...
function initTabs() {
    var proPc = parseInt(localStorage.getItem('directionary_pro_playCount') || 0);
    var proplusPc = parseInt(localStorage.getItem('directionary_proplus_playCount') || 0);
    var isReturning = (proPc + proplusPc) > 0;
    if (isReturning) {
        switchToGameTab();
    } else {
        switchToInfoTab();
...
window.switchToGameTab = switchToGameTab;
